...
class ChatAI {
    ...
    getMessage() {
        ...
        sendChatMessage(chatHistoryId, newMessage).then(response => response.json()).then(data => {
            if (data.error) {
    ...
    createNewConversation(title = null) {  
        ...
        this.container.querySelector('.message-form input').focus();
        this.updateTitle();
        createChatHistory(getUserId(), 'Conversation ' + (this.conversations.length + 1))
        .then(result => {
            console.log(result);
          })
          .catch(error => {
    ...
    _welcomePageTemplate() {
        return `
            <div class="welcome">
                <h1>Posy chatbot</h1>                    
                <p>Start your chat with me... How are you today?</p>
            </div>
        `;
    }
